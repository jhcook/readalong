 ...
 * Returns a cloned element with Shadow DOM content expanded into children.
 */
export function flattenNode(node: Node | null, depth: number = 0, maxDepth: number = 500): Node | null {
    if (!node) return null;
...
export function cleanExtractedHtml(html: string): string {
    ...
    if (paragraphs.length > 0) {
        ...
        for (let i = 1; i < paragraphs.length; i++) {
            // Log vicinity to see what's happening
            ...
            // Check if found sibling matches curr
            if (nextElement === curr) {
...
export function sanitizeContent(html: string): string {
    ...
    return DOMPurify.sanitize(html, {
        ALLOWED_TAGS: ['p', 'b', 'i', 'em', 'strong', 'h1', 'h2', 'h3', 'br', 'ul', 'ol', 'li', 'blockquote', 'div', 'img'],
        ALLOWED_ATTR: ['src', 'alt', 'title'] // Added img support just in case, though sanitized out most times
    });
}
